                ...
                <?php if(is_array($results->all()) && count($results->all())): ?>
                    ...
                    <table id="datatable2" class="table display ">
                        ...
                        <tr>
                            ...
                            <th class="wd-5p"><span>Wanted Time</span></th>
                            <th class="wd-5p"><span>Description</span></th>
                            <th class="wd-5p"><span>Created At</span></th>
                            <th class="wd-5p"><span>Is Accepted</span></th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php foreach ($results as $key => $item): ?>
                            ...
                            <tr id="row{{$item->id}}">
                                ...
                                <td>
                                    <?php if(havePermission("admin/delay_early_requests", "accept_action")): ?>

                                        <?php if ($item->req_status == "pending"): ?>
                                            <?php
                                                echo generate_multi_accepters(
                                                    $accepturl = url("admin/delay-early-requests/change-delay-early-request-status"),
                                                    $item_obj = $item,
                                                    $item_primary_col = "id",
                                                    $accept_or_refuse_col = "req_status",
                                                    $model = App\models\hr\hr_delay_early_requests_m::class,
                                                    $accepters_data = [
                                                        "pending"  => "Pending",
                                                        "rejected" => "Reject",
                                                        "accepted" => "Accept"
