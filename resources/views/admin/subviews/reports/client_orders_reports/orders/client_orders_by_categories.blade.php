 ...
 * @var \Illuminate\Support\Collection $sum_orders_category
    ...
    <div class="container-fluid">
        ...
        <div class="slim-pageheader">
            <ol class="breadcrumb slim-breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Sales Count By Categories</li>
            </ol>
            <h6 class="slim-pagetitle">Sales Count By Categories {{$report_type_date}}</h6>
        </div><!-- slim-pageheader -->
        ...
        <a href="{{url("admin/reports/orders-items-categories-quantities-yearly")}}" class="btn btn-info" >Yearly</a>
        <a href="{{url("admin/reports/orders-items-categories-quantities-monthly")}}" class="btn btn-info" >Monthly</a>
        <a href="{{url("admin/reports/orders-items-categories-quantities?date_from=" . $dayFrom7Pasts . "&date_to=" . date("Y-m-d"))}}" class="btn btn-info" >Weekly</a>
        <a href="{{ url("admin/reports/orders-items-categories-quantities?date_from=" . date("Y-m-d") . "&date_to=" . date("Y-m-d"))}}" class="btn btn-info" >Daily</a>
        <p class="mb-2"></p>
                        ...
                        <tbody>
                            <?php foreach ($report_sold_products_buy as $key => $item): ?>
                                <?php
                                    ...
                                    $quantity       = (int)$item->order_quantity_sum - (int)($returnQuantity->order_quantity_sum ?? 0);

                                    $SUMOrdersCategoryChartData[$item->item_name] = $quantity;
        ...
        @if(isset($SUMOrdersCategoryChartData))
            <div class="section-wrapper mb-3">
                <div class="row">
                    <div class="col-md-12">
                        <div id="{{$report_type_date}} _sold_products_chart_id" data-value="{{json_encode($SUMOrdersCategoryChartData)}}" data-title="Sold Products {{$report_type_date}} " class="google_chart draw_bar_chart" style="height: 600px;"></div>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if(isset($orders_category) && isset($report_sold_products_buy))
            <div class="section-wrapper mb-3">
                <div class="row">
                    <div class="col-md-6">
                        <?php
                            $totalSoldOnly = $report_sold_products_buy->
                            pluck("order_quantity_sum", "item_name")->
                            where("operation_type",'return')->
                            all();
                        ?>
