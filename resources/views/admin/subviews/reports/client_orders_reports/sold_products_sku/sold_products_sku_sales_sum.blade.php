    ...
    <div class="container-fluid">
        <div class="slim-pageheader">
            <ol class="breadcrumb slim-breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Sales Sum By Products SKUS</li>
            </ol>
            <h6 class="slim-pagetitle">Sales Sum By Products SKUS {{$report_type_date}}</h6>
        </div><!-- slim-pageheader -->
        ...
        <a href="{{url("admin/reports/sold-products-sku-sales-sum-yearly")}}" class="btn btn-info" >Yearly</a>
        <a href="{{url("admin/reports/sold-products-sku-sales-sum-monthly")}}" class="btn btn-info" >Monthly</a>
        <a href="{{url("admin/reports/sold-products-sku-sales-sum?date_from=" . $dayFrom7Pasts . "&date_to=" . date("Y-m-d"))}}" class="btn btn-info" >Weekly</a>
        <a href="{{ url("admin/reports/sold-products-sku-sales-sum?date_from=" . date("Y-m-d") . "&date_to=" . date("Y-m-d"))}}" class="btn btn-info" >Daily</a>
        <p class="mb-2"></p>
                    ...
                    <table id="datatable2" class="table display ">
                        ...
                        <tbody>
                            ...
                            <?php foreach ($report_sold_products_buy as $key => $item): ?>
                                <?php
                                    $sold_product = $report_sold_products_return->
                                    where("item_name", $item->item_name)->
                                    where("operation_type", 'return')->
                                    first();

                                    $total_amount = $item->total_items_cost - ($sold_product->total_items_cost ?? 0);

                                    $soldProductsChartData[$item->item_name] = $total_amount;
                                ?>
